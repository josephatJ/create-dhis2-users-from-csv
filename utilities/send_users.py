...
async def check_if_user_exist(payload, DEST_BASE_URL,username,password,headers):
    ...
    response = requests.get(DEST_BASE_URL + '/api/account/username?username=' + payload['userCredentials']['username'], auth=(username,password), verify=False)
    if response.status_code != 200 and response.status_code != 201 and json.loads(response.content.decode('utf-8'))["response"] == "error":
        print("User found.........")
        return True
    else:
        return False
